<template>
  <n-card>
    <n-space vertical>
      <n-h2>Courier Zone Masking</n-h2>
      <n-divider>Create courier zone masking</n-divider>
      <n-space>
        <n-space>
          <p style="font-size: 20px">Select Courier</p>
          <n-select
            v-model:value="selectedCourierID"
        ...
        <n-space>
          <p style="font-size: 20px">Select Zone</p>
          <n-select
        ...
        <n-space>
          <p style="font-size: 20px">Zone Masking</p>
          <n-input v-model:value="masking" type="text" placeholder="Masking" />
        </n-space>
        <n-button @click="saveMasking" type="success">Save</n-button>
      </n-space>
      <n-divider>Courier zone masking list</n-divider>
      <n-table :bordered="false" :single-line="false">
        <thead>
          <tr>
            ...
            <th style="font-weight: bold">Masking</th>
            <th style="font-weight: bold">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in zoneMaskings" :key="index">
            <td>{{ item.id }}</td>
            ...
            <td>
              <n-space>
                ...
                <n-popconfirm @positive-click="handlePositiveClick(item)">
                  ...
                  Are you sure, you want to delete "{{ item.masking }}"?
                </n-popconfirm>
              </n-space>
            </td>
          </tr>
...
<script lang="ts" setup>
  ...
  import {
    courierList,
    createCourierZoneMasking,
    deleteCourierZoneMasking,
    getCourierZoneMaskingList,
    getZonesList,
  } from '@/api/courier';

  const zoneMaskings = ref();
  const selectedCourierID: any = ref();
  const selectedZoneID: any = ref();
  const masking: any = ref();
  const courierOptions: any = ref();
  const zoneOptions: any = ref();
  onMounted(() => {
    getList();
    courierList('').then(({ result }) => {
  ...
  function saveMasking() {
    if (!selectedCourierID.value) {
      ...
      window['$message'].error('Please select zone');
    } else if (!masking.value) {
      window['$message'].error('Please enter masking');
    } else {
      const data = {
  ...
  function deleteItem(item) {
    deleteCourierZoneMasking(item.id)
      .then(({ result }) => {
        window['$message'].success(result.message);
        getList();
      })
  ...
  function getList() {
    getCourierZoneMaskingList().then(({ result }) => {
      zoneMaskings.value = result.data;
    });
  }
</script>
