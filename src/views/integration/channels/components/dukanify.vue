        ...
        <n-grid-item :span="2">
          <n-card style="border-radius: 12px; background: rgb(235, 237, 247); padding: 12px">
            <n-space :vertical="true">
              ...
              >
              <p style="font-size: 18px; font-weight: 600; color: rgb(38, 63, 151)"
                >Shop token - {{ formValue.weship_secret_id }}
              </p>
            </n-space>
          </n-card>
        </n-grid-item>
                ...
                <n-space :vertical="true">
                  <n-space style="margin-left: 10px">
                    <n-space :vertical="true">
                      ...
                      >
                        <n-input
                          v-model:value="formValue.website_url"
                          placeholder="eg: https//fredefy.myDukanify.com/admin"
                        />
                      </n-form-item>
                    </n-space>
                    <!--                    <p-->
                    ...
                    <!--                      "-->
                    <!--                    >-->
                    <!--                      Enter the website admin url-->
                    <!--                    </p>-->
                  </n-space>
                  <n-button
                    :round="true"
                    type="info"
                    @click="nextBtn(formValue.website_url)"
                    style="
                      font-weight: bold;
                      font-size: 14px;
                      text-align: center;
            ...
            <n-step>
              <n-space :vertical="true" style="margin-left: 30px">
                <n-space>
                  ...
                  <input id="myInput" :value="userStore.getToken" hidden />
            ...
            <n-step>
              <n-space :vertical="true">
                <p
                  ...
                  "
                >
                  Once Dukanify Website Admin url entered, click on connect with Dukanify button
                  below.
                </p>
                <n-button
                  :round="true"
                  type="info"
                  @click="connectWithDukanify"
                  style="
                    margin-left: 20px;
                    font-weight: bold;
                    font-size: 14px;
              ...
              <div style="display: flex; flex-direction: row; margin-top: 10px">
                ...
                <p
                  style="
                    ...
                    padding-bottom: 8px;
                  "
                  >Enter website admin url of Dukanify store and click on Download Weship App.</p
                >
              </div>
              <div style="display: flex; flex-direction: row">
                ...
                <p
                  style="
                    ...
                    padding-bottom: 8px;
                  "
                  >You’ll be re-directed to Login Dukanify admin panel. On successful login, you’ll
                  need to install Weship app and update Weship auth token for necessary approval.
                </p>
              </div>
              <div style="display: flex; flex-direction: row">
                <p
                ...
                <p
                  style="
                    ...
                    padding-bottom: 8px;
                  "
                  >Once Weship app is configured on Dukanify, come back to Weship and click on
                  Connect with Dukanify button. System will automatically fetch authentication token
                  from Dukanify.
                </p>
              </div>
              <div style="display: flex; flex-direction: row">
          ...
          >
            ...
            <n-form-item :show-label="false" style="margin-bottom: 0px">
              <n-input
                ...
                placeholder="Enter the initial status of Dukanify Orders on to sync in Weship"
              />
            </n-form-item>
            <p
              style="
            ...
            <p
              ...
              "
            >
              Update Dukanify status codes against provided Weship Tracking Statuses
            </p>
            <p
              style="
                font-weight: bold;
            ...
            <n-form-item path="order_placed" :show-label="false" style="margin-bottom: 0px">
              <n-input
                ...
                placeholder="Enter the exact status on Dukanify when order is placed in Weship"
              />
            </n-form-item>
            <p
              style="
            ...
            <n-form-item path="in_transit" :show-label="false" style="margin-bottom: 0px">
              <n-input
                ...
                placeholder="Enter the exact status on Dukanify when order is in transit to customers"
              />
            </n-form-item>
            <p
              style="
            ...
            <n-form-item path="rtd" :show-label="false" style="margin-bottom: 0px">
              <n-input
                v-model:value="formValue.rtd"
                placeholder="Enter the exact status on Dukanify when order is RTD to end customer"
              />
            </n-form-item>
            <p
              style="
            ...
            <n-form-item path="rto" :show-label="false" style="margin-bottom: 0px">
              <n-input
                ...
                placeholder="Enter the exact status on Dukanify when order is RTO to end customer"
              />
            </n-form-item>
          </n-form>
        </n-grid-item>
          ...
          >
            ...
            <n-form-item path="picked_up" :show-label="false" style="margin-bottom: 0px">
              <n-input
                ...
                placeholder="Enter the exact status on Dukanify when order is picked to end customer"
              />
            </n-form-item>
            <p
              style="
            ...
            <n-form-item path="delivered" :show-label="false" style="margin-bottom: 0px">
              <n-input
                ...
                placeholder="Enter the exact status on Dukanify when order is delivered to end customer"
              />
            </n-form-item>
            <p
              style="
            ...
            <n-form-item path="delivered" :show-label="false" style="margin-bottom: 0px">
              <n-input
                ...
                placeholder="Enter the exact status on Dukanify when order is cancelled to end customer"
              />
            </n-form-item>
          </n-form>
        </n-grid-item>
...
<script lang="ts" setup>
  ...
  import { getApiToken } from '@/api/channels/shopify';
  ...
  getApiToken({ name: 'dukanify' }).then(({ result }) => {
  ...
  const rules = {
    website_url: {
      ...
      validator(rule: FormItemRule, value: string) {
        if (!value) {
          return new Error('Dukanify website admin url is required');
        } else if (
    ...
    formRef.value?.validate((errors) => {
      if (!errors) {
        if (currentRef.value === 1) {
          domainOnly = removeHttp(url);
          // console.log(removeHttp(url));
          currentRef.value = 2;
        }
      } else {
        // console.log(errors);
        window['$message'].error('Please fill out website admin url');
      }
    });
  }
  ...
  function connectWithDukanify() {
    if (currentRef.value === 3) {
      const Data = {
        ...
        dukanify_token_id: tokenInfo.value.token_id,
        is_active: true,
      };
      ...
      createNewMerchantChannelMapping(Data)
        .then(({ result }) => {
          ...
          //       if (shopMetaDataParameters.value[param.key]) {
          //         shopMetaDataParameters.value[param.key].value = param.value;
          //         shopMetaDataParameters.value[param.key].id = param.id;
          //       }
          //     }
  ...
  function saveData() {
    if (isChannelConnected.value) {
      ...
      window['$message'].error('Please Connect with Dukanify first');
    }
  }
  function WeShipAuthTokenCopied() {
    if (currentRef.value === 2) {
