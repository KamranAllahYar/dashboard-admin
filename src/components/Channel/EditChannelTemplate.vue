...
<script lang="ts" setup>
  ...
  import { getChannelTemplateDetailsById, updateChannelTemplate } from '@/api/channels/template';
  ...
  getChannelTemplateDetailsById(props.id).then(({ result }) => (formValue.value = result.data));
  ...
  function update() {
    formRef.value?.validate((errors) => {
      if (!errors) {
        ...
        Loading.start();
        updateChannelTemplate(props.id, formValue.value)
          .then(({ result }) => {
