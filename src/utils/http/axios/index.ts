...
const transform: AxiosTransform = {
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    if (!isCancel) {
      checkStatus(error.response && error.response.status, msg, error.response.data);
    } else {
      console.warn(error, 'The request was cancelled!');
    }
    //return Promise.reject(error);
    return Promise.reject({ result: response?.data, code, message });
  },
};
...
export const httpTwo = (prefix = '') => {
  prefix = `${urlPrefix}/${prefix}`;
  return createAxios({ withCredentials: true, requestOptions: { urlPrefix: prefix } });
};
...
export const httpDownload = createAxios({
  withCredentials: true,
  authenticationScheme: 'bearer',
  requestOptions: {
